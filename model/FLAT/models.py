...
class Absolute_SE_Position_Embedding(nn.Module):
    ...
    def __init__(
        ...
        if self.fusion_func == 'nonlinear_concat':
            self.pos_proj = nn.Sequential(
                ...
                nn.LeakyReLU(), nn.Linear(self.hidden_size, self.hidden_size))
    ...
    def forward(self, inp, pos_s, pos_e):
        batch = inp.size(0)
        max_len = inp.size(1)
        pe_s = self.pe_s.index_select(0,
                                      pos_s.view(-1)).view(batch, max_len, -1)
        pe_e = self.pe_e.index_select(0,
                                      pos_e.view(-1)).view(batch, max_len, -1)
...
class Absolute_Position_Embedding(nn.Module):
    ...
    def forward(self, inp):
        ...
        if self.fusion_func == 'add':
            ...
            inp = torch.cat([
                inp, self.pe[:, :inp.size(1)].repeat([batch] + [1] *
                                                     (inp.dim() - 1))
            ],
                            dim=-1)
            output = self.proj(inp)
