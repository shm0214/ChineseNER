...
output_f = open(yangjie_rich_pretrain_char_and_word_path,
                'w',
                encoding='utf-8')
...
def create_cliped_file(fp):
    ...
    for line in lines:
        ...
        if len(line_split) == 0 or \
            ...
            now_example_len = 0
               ...
               (now_example_len > 1 and last_line_split[0] == '…'
                and line_split[0] == '…')) and line_split[1] == 'O'
              and now_example_len > 210):
            print('', file=f_out)
            now_example_len = 0
    ...
    for example in cliped_examples:
        if len(example) > 200:
            print(len(example), ''.join(map(lambda x: x.split(' ')[0],
                                            example)))
            check = 1
