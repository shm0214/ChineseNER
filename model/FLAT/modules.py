...
class Four_Pos_Fusion_Embedding(nn.Module):
    ...
    def __init__(self, pe, four_pos_fusion, pe_ss, pe_se, pe_es, pe_ee,
        ...
        if self.four_pos_fusion == 'ff_linear':
            ...
            self.pos_attn_score = nn.Sequential(
                ...
                nn.ReLU(), nn.Linear(self.hidden_size * 4, 4),
                nn.Softmax(dim=-1))
    ...
    def forward(self, pos_s, pos_e):
        ...
        pe_ss = self.pe_ss[(pos_ss).view(-1) + self.max_seq_len].view(
            size=[batch, max_seq_len, max_seq_len, -1])
        pe_se = self.pe_se[(pos_se).view(-1) + self.max_seq_len].view(
            size=[batch, max_seq_len, max_seq_len, -1])
        pe_es = self.pe_es[(pos_es).view(-1) + self.max_seq_len].view(
            size=[batch, max_seq_len, max_seq_len, -1])
        pe_ee = self.pe_ee[(pos_ee).view(-1) + self.max_seq_len].view(
            size=[batch, max_seq_len, max_seq_len, -1])

        if self.four_pos_fusion == 'ff':
            pe_4 = torch.cat([pe_ss, pe_se, pe_es, pe_ee], dim=-1)
            if self.mode['gpumm']:
                print('四个位置合起来:{},{}'.format(pe_4.size(),
                                             size2MB(pe_4.size())))
        ...
        if self.four_pos_fusion == 'ff_two':
            ...
            if self.mode['gpumm']:
                print('2个位置合起来:{},{}'.format(pe_2.size(),
                                             size2MB(pe_2.size())))
            rel_pos_embedding = self.pos_fusion_forward(pe_2)
        elif self.four_pos_fusion == 'attn':
            pe_4 = torch.cat([pe_ss, pe_se, pe_es, pe_ee], dim=-1)
            attn_score = self.pos_attn_score(pe_4)
            pe_4_unflat = self.w_r(
                pe_4.view(batch, max_seq_len, max_seq_len, 4,
                          self.hidden_size))
...
class MultiHead_Attention_Lattice_rel_save_gpumm(nn.Module):
    ...
    def forward(self, key, query, value, seq_len, lex_num, pos_s, pos_e,
        ...
        query_for_b_and_v_for_d = query_for_b + self.v.view(
            1, self.num_heads, 1, 1, self.per_head_size)
        ...
        mask = seq_len_to_mask(seq_len +
                               lex_num).bool().unsqueeze(1).unsqueeze(1)
...
class MultiHead_Attention_Lattice_rel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.four_pos_fusion == 'ff':
            ...
            self.pos_attn_score = nn.Sequential(
                ...
                nn.ReLU(), nn.Linear(self.hidden_size * 2, 4),
                nn.Softmax(dim=-1))
    ...
    def forward(self, key, query, value, seq_len, lex_num, pos_s, pos_e):
        ...
        pe_ss = self.pe[(pos_ss).view(-1) + self.max_seq_len].view(
            size=[batch, max_seq_len, max_seq_len, -1])
        pe_se = self.pe[(pos_se).view(-1) + self.max_seq_len].view(
            size=[batch, max_seq_len, max_seq_len, -1])
        pe_es = self.pe[(pos_es).view(-1) + self.max_seq_len].view(
            size=[batch, max_seq_len, max_seq_len, -1])
        pe_ee = self.pe[(pos_ee).view(-1) + self.max_seq_len].view(
            size=[batch, max_seq_len, max_seq_len, -1])
        ...
        if self.four_pos_fusion == 'ff':
            ...
            pe_4 = torch.cat([pe_ss, pe_se, pe_es, pe_ee], dim=-1)
            attn_score = self.pos_attn_score(pe_4)
            pe_4_unflat = pe_4.view(batch, max_seq_len, max_seq_len, 4,
                                    self.hidden_size)
        ...
        D_ = torch.matmul(rel_pos_embedding_for_d,
                          v_for_d).squeeze(-1).squeeze(-1).permute(0, 3, 1, 2)
...
class MultiHead_Attention_rel(nn.Module):
    ...
    def forward(self, key, query, value, seq_len):
        ...
        rel_pos_embedding = rel_pos_embedding_flat.view(
            size=[max_seq_len, max_seq_len, self.hidden_size])
        ...
        rel_pos_embedding_for_b = rel_pos_embedding.unsqueeze(0).permute(
            0, 3, 1, 4, 2)
        query_for_b = query.view(
            [batch, self.num_heads, max_seq_len, 1, self.per_head_size])
        B_ = torch.matmul(query_for_b, rel_pos_embedding_for_b).squeeze(-2)
        ...
        v_for_d = self.v.unsqueeze(-1)
        D_ = torch.matmul(rel_pos_embedding_for_d,
                          v_for_d).squeeze(-1).squeeze(-1).permute(
                              2, 0, 1).unsqueeze(0)
...
class Absolute_Position_Embedding(nn.Module):
    ...
    def __init__(self,
        ...
        if self.mode['debug']:
            ...
            print_info('pe avg:{}'.format(
                torch.sum(self.pe) / (self.pe.size(2) * self.pe.size(1))))
...
class Transformer_Encoder_Layer(nn.Module):
    ...
    def __init__(self,
                 ...
                 four_pos_fusion=None,
                 four_pos_fusion_embedding=None,
                 use_pytorch_dropout=True):
        super().__init__()
        self.use_pytorch_dropout = use_pytorch_dropout
        ...
        if self.relative_position:
            if pe is None:
                ...
                self.pe = nn.Parameter(pe,
                                       requires_grad=self.learnable_position)
        ...
        self.layer_preprocess = Layer_Process(self.layer_preprocess_sequence,
                                              self.hidden_size,
                                              self.dropout['pre'],
                                              self.use_pytorch_dropout)
        self.layer_postprocess = Layer_Process(self.layer_postprocess_sequence,
                                               self.hidden_size,
                                               self.dropout['post'],
                                               self.use_pytorch_dropout)
        if self.relative_position:
            if not self.lattice:
                self.attn = MultiHead_Attention_rel(
                    self.hidden_size,
                    self.num_heads,
                    pe=self.pe,
                    scaled=self.scaled,
            ...
            self.attn = MultiHead_Attention(self.hidden_size,
                                            self.num_heads,
                                            self.scaled,
                                            mode=self.mode,
                                            k_proj=self.k_proj,
        ...
        self.ff = Positionwise_FeedForward(
            [hidden_size, ff_size, hidden_size],
            self.dropout,
        ...
        if self.lattice:
            if self.relative_position:
                if rel_pos_embedding is None:
                    rel_pos_embedding = self.four_pos_fusion_embedding(
                        pos_s, pos_e)
