...
from model.softLexicon_LatticeLSTM.LatticeLSTM import LatticeLSTM
...
class BiLSTM(nn.Module):
    def __init__(self, config):
        ...
        self.forward_lstm = LatticeLSTM(lstm_input_dim, lstm_hidden_dim, config.gaz_dropout, config.gaz_alphabet.size(), config.gaz_embedding_dim, config.pretrain_gaz_embedding, True, config.fix_gaz_embedding, self.use_cuda, config)
        if self.use_bilstm:
            self.backward_lstm = LatticeLSTM(lstm_input_dim, lstm_hidden_dim, config.gaz_dropout, config.gaz_alphabet.size(), config.gaz_embedding_dim, config.pretrain_gaz_embedding, False, config.fix_gaz_embedding, self.use_cuda, config)
    ...
    def get_lstm_features(self, gaz_list, word_inputs, biword_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover):
        ...
        if self.use_bilstm:
            ...
            backward_lstm_out, backward_hidden = self.backward_lstm(word_embeddings, gaz_list, backward_hidden)
